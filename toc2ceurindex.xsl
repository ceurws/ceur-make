...
<html xmlns="http://www.w3.org/1999/xhtml"
    ...
    <head>
        ...
        <xsl:text disable-output-escaping="yes"><![CDATA[<script src="http://code.jquery.com/jquery-2.1.3.min.js"></script><script src="http://linked-research.270a.info/scripts/html.sortable.min.js"></script><script src="http://linked-research.270a.info/scripts/lr.js"></script>]]></xsl:text>
    </head>
    ...
    <body about="[this:]" typeof="schema:Article bibo:Proceedings sioc:Post prov:Entity" class="h-feed" prefix="rdf: http://www.w3.org/1999/02/22-rdf-syntax-ns# rdfs: http://www.w3.org/2000/01/rdf-schema# owl: http://www.w3.org/2002/07/owl# xsd: http://www.w3.org/2001/XMLSchema# dcterms: http://purl.org/dc/terms/ foaf: http://xmlns.com/foaf/0.1/ v: http://www.w3.org/2006/vcard/ns# pimspace: http://www.w3.org/ns/pim/space# skos: http://www.w3.org/2004/02/skos/core# prov: http://www.w3.org/ns/prov# schema: http://schema.org/ sioc: http://rdfs.org/sioc/ns# rsa: http://www.w3.org/ns/auth/rsa# cert: http://www.w3.org/ns/auth/cert# cal: http://www.w3.org/2002/12/cal/ical# wgs: http://www.w3.org/2003/01/geo/wgs84_pos# bibo: http://purl.org/ontology/bibo/ dbr: http://dbpedia.org/resource/ dbp: http://dbpedia.org/property/ sio: http://semanticscience.org/resource/ opmw: http://www.opmw.org/ontology/ deo: http://purl.org/spar/deo/ doco: http://purl.org/spar/doco/ cito: http://purl.org/spar/cito/ fabio: http://purl.org/spar/fabio/ this: {$volume-url}">
        <header>
            ...
            <dl id="document-identifier">
                ...
                <dt>URN</dt>
                <dd property="bibo:uri" xml:lang="" lang="">urn:nbn:de:0074-<xsl:value-of select="$number"/>-C</dd>
                <dt>Volume</dt>
                <dd property="bibo:volume" xml:lang="" lang=""><xsl:value-of select="$volume"/></dd>
            </dl>

            <dl id="document-license">
                <dt>Copyright</dt>
                <dd>© <xsl:value-of select="$pubyear"/> for the individual papers by the papers' authors. Copying permitted for private and academic purposes. This volume is published and copyrighted by its editors.</dd>
            </dl>
        </header>

        <article class="h-entry">
            <h1><a rel="foaf:homepage" href="{ $workshop/homepage }" property="bibo:shortTitle schema:alternateName"><xsl:value-of select="$workshop/title/acronym"/><xsl:text> </xsl:text><xsl:value-of select="$year"/></a><xsl:text> </xsl:text><span class="p-name" property="schema:name"><xsl:value-of select="$workshop/title/volume"/></span></h1>
                ...
                <dl id="document-event" class="h-event" about="[this:]" rel="bibo:presentedAt" resource="[this:#event]">
                    ...
                    <dd class="p-summary" property="schema:description">
                        <span class="p-name" property="schema:name"><xsl:value-of select="$workshop/title/full"/></span>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:comment> co-located with &lt;span&gt;NONE&lt;/span&gt; </xsl:comment>
                            </xsl:otherwise>
                        </xsl:choose>
                    </dd>
                    <dt>Location</dt>
                    <dd class="p-location" rel="schema:location" resource="{ replace($workshop/location/@href, 'https?://en\.wikipedia\.org/wiki/', 'http://dbpedia.org/resource/') }"><xsl:value-of select="$workshop/location"/></dd>
                    <dt>Date</dt>
                    <dd>
                        <xsl:choose>
            ...
            <div id="authors">
                <dl id="author-name">
                    ...
                    <xsl:for-each select="$workshop/editors/editor">
                        ...
                        <dd id="author-{position()}" rel="bibo:editor" resource="{$editorIRI}"><span about="[this:]" rel="schema:contributor schema:author schema:editor{if (@submitting='true') then ' schema:creator' else ''}"><a about="{$editorIRI}" typeof="schema:Person" rel="schema:url" property="schema:name" href="{homepage}"><xsl:value-of select="name"/></a></span><sup><a about="{$editorIRI}" rel="schema:memberOf" resource="[this:#{replace(normalize-space(affiliation), '\s+', '')}]" href="#author-org-{position()}"><xsl:value-of select="position()"/></a></sup></dd>
                    </xsl:for-each>
                </dl>
                    ...
                    <div about="[this:#table-of-contents]" property="schema:description">
                        <ul>
                            ...
                            <li id="{$id}" about="[this:#{$id}]" typeof="schema:ScholarlyArticle">
                                <a about="[this:#{$id}]" rel="dcterms:hasFormat" property="schema:name" href="{ resolve-uri($pdf, $volume-url) }"><xsl:value-of select="title"/></a>
                                <xsl:if test="pages">
                                <dl class="pages">
                                    <dt>Pages</dt>
                                    <dd><span about="[this:#{$id}]" property="bibo:pageStart"  datatype="xsd:nonNegativeInteger"><xsl:value-of select="pages/@from"/></span>–<span about="[this:#{$id}]" property="bibo:pageEnd" datatype="xsd:nonNegativeInteger"><xsl:value-of select="pages/@to"/></span></dd>
                                </dl>
                                </xsl:if>
                                <dl class="authors">
                                    <dt>Authors</dt>
                                    ...
                                    <dd id="{$id}-{$authorIRI}" rel="bibo:authorList" inlist="" resource="[this:#{$id}-{$authorIRI}]"><span about="[this:#{$id}]" rel="schema:author"><span about="[this:#{$id}-{$authorIRI}]" typeof="schema:Person" property="schema:name"><xsl:value-of select="normalize-space(.)"/></span></span></dd>
                                    </xsl:for-each>
                                </dl>
                            </li>
                            </xsl:for-each>
        ...
        <footer>
            ...
            <p><time datetime="YYYY-MM-DD"  property="schema:datePublished" content="YYYY-MM-DD" datatype="xsd:date">YYYY-MM-DD</time>: published on CEUR-WS.org</p>
        </footer>
    </body>
</html>
    </xsl:template>
